...
export default function HomePage() {
  ...
  return (
    <main className={isLight ? "site-shell light" : "site-shell"} data-testid="home-page">
      ...
      <section className="contact-section section-pad" id="contact" data-testid="contact-section"><div className="contact-copy"><p className="eyebrow">Start a conversation</p><h2>Let’s make your next digital chapter feel unmistakably yours.</h2><p data-testid="contact-details">Contact details are ready to be tailored with Qwebliq’s phone, WhatsApp, email, location, and working hours.</p><a className="whatsapp-link" data-testid="whatsapp-contact-link" href="https://wa.me/" rel="noreferrer" target="_blank">WhatsApp us <ArrowUpRight size={16} /></a></div><form className="inquiry-form" data-testid="inquiry-form" onSubmit={sendInquiry}><input data-testid="inquiry-name-input" name="name" placeholder="Your name" required /><input data-testid="inquiry-email-input" name="email" placeholder="Email address" required type="email" /><input data-testid="inquiry-company-input" name="company" placeholder="Company or organisation" /><select data-testid="inquiry-budget-select" name="budget"><option value="">Indicative investment</option><option>₹50k–₹1L</option><option>₹1L–₹3L</option><option>₹3L+</option></select><textarea data-testid="inquiry-message-input" minLength="10" name="message" placeholder="Tell us what you’re building" required /><button className="button button-primary" data-testid="inquiry-submit-button" type="submit">Send project note <ArrowUpRight size={17} /></button></form></section>
