  ...
  return (
    <main className={isLight ? "site-shell light" : "site-shell"} data-testid="home-page">
      ...
      <section className="work-section section-pad" id="work" data-testid="work-section">
        ...
        {content.projects.map((project, index) => <article className="feature-project" data-testid={`project-${project.slug || "item"}-${index}`} key={`${project.id || project.slug || project.title}-${index}`}><div className="project-image"><img alt={`${project.title} project preview`} data-testid={`project-image-${project.slug || "item"}-${index}`} src={project.cover_image} /></div><div className="project-details"><p className="eyebrow">{project.industry} / {project.year}</p><h3>{project.title}</h3><p>{project.summary}</p><div className="project-tags">{project.technologies?.map((technology) => <span data-testid={`technology-${technology}`} key={technology}>{technology}</span>)}</div><a className="text-link" data-testid={`project-live-${project.slug || "item"}-${index}`} href={project.live_url} rel="noreferrer" target="_blank">Visit live website <ArrowUpRight size={16} /></a></div></article>)}
      </section>
