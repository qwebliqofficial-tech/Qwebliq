...
export default function HomePage() {
  ...
  return (
    <main className={isLight ? "site-shell light" : "site-shell"} data-testid="home-page">
      ...
      <section className="calculator-section section-pad" data-testid="calculator-section"><div><p className="eyebrow">Project planner</p><h2>A useful starting point, not a fixed quote.</h2><p>Choose a few signals and get a clear opening range for your conversation with us.</p></div><form className="calculator-form" data-testid="project-calculator-form" onSubmit={calculate}><label>Focus<select data-testid="calculator-project-type" defaultValue="website" name="project_type"><option value="website">Growth website</option><option value="ecommerce">E-commerce</option><option value="brand">Brand system</option><option value="growth">Growth programme</option></select></label><label>Timeline<select data-testid="calculator-timeline" defaultValue="standard" name="timeline"><option value="standard">Thoughtful pace</option><option value="accelerated">Accelerated</option></select></label><label>Pages<input data-testid="calculator-pages" defaultValue="6" max="50" min="1" name="pages" type="number" /></label><button className="button button-primary" data-testid="calculator-submit-button" type="submit">Calculate range <ArrowUpRight size={17} /></button>{estimate && <strong className="estimate" data-testid="calculator-result">{estimate}</strong>}</form></section>
      ...
      <section className="contact-section section-pad" id="contact" data-testid="contact-section"><div className="contact-copy"><p className="eyebrow">Start a conversation</p><h2>Let’s make your next digital chapter feel unmistakably yours.</h2><p data-testid="contact-details">Call <a data-testid="contact-phone-primary-link" href="tel:+919774090507">+91 97740 90507</a> or <a data-testid="contact-phone-secondary-link" href="tel:+919362823252">+91 93628 23252</a>. You can also write to <a data-testid="contact-email-link" href="mailto:qwebliqofficial@gmail.com">qwebliqofficial@gmail.com</a>.</p><a className="whatsapp-link" data-testid="whatsapp-contact-link" href="https://wa.me/919774090507?text=Hello%20Qwebliq%2C%20I%20would%20like%20to%20discuss%20a%20project." rel="noreferrer" target="_blank">WhatsApp Qwebliq <ArrowUpRight size={16} /></a></div><form className="inquiry-form" data-testid="inquiry-form" onSubmit={sendInquiry}><input data-testid="inquiry-name-input" name="name" placeholder="Your name" required /><input data-testid="inquiry-email-input" name="email" placeholder="Email address" required type="email" /><input data-testid="inquiry-company-input" name="company" placeholder="Company or organisation" /><select data-testid="inquiry-budget-select" name="budget"><option value="">Indicative investment</option><option>₹50k–₹1L</option><option>₹1L–₹3L</option><option>₹3L+</option></select><textarea data-testid="inquiry-message-input" minLength="10" name="message" placeholder="Tell us what you’re building" required /><button className="button button-primary" data-testid="inquiry-submit-button" type="submit">Send project note <ArrowUpRight size={17} /></button></form></section>
